...
const TimerContainer = styled.div`
  ...
  color: ${(props) =>
    props.isExpiring ? "rgb(220, 38, 38)" : "var(--color-text)"};
...
const TimerIcon = styled.span`
  ...
  color: ${(props) =>
    props.isExpiring ? "rgb(220, 38, 38)" : "var(--color-primary)"};
  font-size: 12px;
`;
  ...
  useEffect(() => {
    ...
    const calculateTimeLeft = () => {
      ...
      if (difference <= 0) {
        ...
        if (auctionId) {
          console.log(
            `Auction timer ended for auction ${auctionId}, completing auction...`
          );
          completeAuction(auctionId)
            .then((result) => {
              ...
              if (result.success) {
                ...
                console.error(
                  `Failed to complete auction ${auctionId}:`,
                  result.error
                );
              }
            })
            .catch((err) => {
              console.error(
                `Error in auction completion for ${auctionId}:`,
                err
              );
            });
        }
      ...
      return (
        <TimeDisplay>
          <TimeUnit compact={compact}>
            <TimeValue compact={compact} isExpiring={isExpiring}>
              {days}
            </TimeValue>
            <TimeLabel compact={compact} isExpiring={isExpiring}>
              Gün
            </TimeLabel>
          </TimeUnit>
