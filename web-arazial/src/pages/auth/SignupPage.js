...
const SignupPage = () => {
  ...
  const handleSubmitInitialForm = async (e) => {
    ...
    if (!acceptTerms) {
      setErrors((prev) => ({
        ...prev,
        terms: "Üyelik sözleşmesini kabul etmelisiniz",
      }));
      return;
    }
    ...
    try {
      ...
      const checkResponse = await fetch(
        `${process.env.REACT_APP_SUPABASE_URL}/functions/v1/check-phone`,
        {
          method: "POST",
          headers: {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (otp.length !== 6) {
      setErrors({
        general: "Lütfen 6 haneli doğrulama kodunu tam olarak girin.",
      });
      return;
    }
    ...
    try {
      ...
      const verifyResponse = await fetch(
        `${process.env.REACT_APP_SUPABASE_URL}/functions/v1/verify-otp`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const renderInitialForm = () => (
    <Form onSubmit={handleSubmitInitialForm}>
      ...
      <CheckboxContainer>
        ...
        />
        <CheckboxLabel>
          Kampanya ve tekliflerden yararlanmak için, E-Posta, SMS vb. iletileri
          ve aramaları almayı kabul ediyorum.
        </CheckboxLabel>
      </CheckboxContainer>
      ...
      <div style={{ textAlign: "center", marginTop: "1rem" }}>
        ...
        >
          Zaten üye misiniz?{" "}
          <span style={{ fontWeight: "bold" }}>Giriş yapın</span>
        </Link>
      </div>
    </Form>
  );
