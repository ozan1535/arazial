        ...
        {(currentStatus === 'active' || currentStatus === 'upcoming') && (
          <ShareButton onClick={handleShare} title="İlanı paylaş">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 5.25v.01M12 18.75v.01M16.883 10.907a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186zM6.117 6.117a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186zM12 12.75a2.25 2.25 0 100-4.186 2.25 2.25 0 000 4.186z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M17.883 6.117a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186zM6.117 17.883a2.25 2.25 0 100-4.186 2.25 2.25 0 000 4.186z" />
            </svg>
          </ShareButton>
        )}
      </BidCardHeader>
          ...
          <>
            ...
            {currentStatus === 'active' && (
              <div style={{ 
                ...
                textAlign: 'center', 
                padding: '1.5rem',
                background: 'linear-gradient(135deg, rgba(var(--color-primary-rgb), 0.03) 0%, rgba(var(--color-primary-rgb), 0.08) 100%)',
                borderRadius: '1rem',
                border: '1px solid rgba(var(--color-primary-rgb), 0.1)',
            ...
            {auction.listing_type === 'auction' && currentStatus !== 'active' && (
              <>
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ color: 'var(--color-text-secondary)', fontSize: '0.875rem' }}>
                        {currentStatus === 'active' ? 'Güncel Teklif:' : 'Başlangıç Fiyatı:'}
                      </span>
                      <strong style={{ fontSize: '1.125rem', color: 'var(--color-text)' }}>
                        {formatPrice(getMinimumBidAmount())}
                      </strong>
                    </div>
                    {auction.minIncrement > 0 && auction.listing_type === 'auction' && (
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ color: 'var(--color-text-secondary)', fontSize: '0.875rem' }}>
                          Minimum Artış:
                        </span>
                 ...
                 <div style={{ 
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ 
                      ...
                      {currentStatus === 'active' ? '⏰ İhale bitimine kalan süre:' : '📅 İhale başlamasına kalan süre:'}
                    </div>
                    <CountdownTimer 
                      endTime={currentStatus === 'active' ? auction.end_time : auction.start_time} 
                      compact={false}
                      onComplete={handleTimerComplete}
                    />
                  </div>
              ...
              <>
                <PaymentWarning>
                  {hasPendingDeposit ? (
                    <>
                      ...
                      <p style={{ marginBottom: 0, textAlign: 'center' }}>Açık artırmayı kazanamamanız durumunda, yatırmış olduğunuz teminat tutarı yedi (7) iş günü içerisinde sistemde kayıtlı banka hesabınıza eksiksiz olarak iade edilecektir.</p>
                    </>
                  ) : (
                    <>
                ...
                <AgreementSection>
                  <AgreementText>
                    ...
                    <p>
                      Hizmet Açıklaması: Hizmet kapsamında, gayrimenkul satışlarına ilişkin Açık Arttırmalar'ın gerçekleştirilebilmesine ve Site Üyelik Sözleşmesi çerçevesinde Site üzerinden yayınlanan gayrimenkul satış ilanlarının incelenmesi sonrasında uygun görülmesi halinde Site üzerinden gerçekleştirilen Açık Arttırmalar'a katılım sağlanmasına yönelik altyapı hizmetleri sağlanmaktadır.
                    </p>
            ...
            currentStatus === 'active' ? (
              auction.end_time && (
                <StartTimeBadge>
                  ...
                  <span className="date-label">Bitiş Tarihi:</span> <ModernDateDisplay date={auction.end_time} compact={true} showIcon={false} />
                </StartTimeBadge>
              )
            ) : (
              auction.start_time && (
                <StartTimeBadge>
                  ...
                  <span className="date-label">Başlangıç Tarihi:</span> <ModernDateDisplay date={auction.start_time} compact={true} showIcon={false} />
                </StartTimeBadge>
              )
            )
          )}
