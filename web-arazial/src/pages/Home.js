...
const CountdownLabel = styled.div`
  ...
  svg {
    width: 1em;
    height: 1em;
    color: ${props => props.status === 'active' ? 'rgb(5, 150, 105)' : 'rgb(37, 99, 235)'};
  }
`;
...
const Home = () => {
  ...
  const getCurrentBid = (listing) => {
    ...
    return listing.highest_bid || listing.final_price || listing.finalPrice || listing.starting_price || listing.startingPrice || listing.starting_bid || 0;
  };
                ...
                <AuctionCard key={listing.id} onClick={() => handleAuctionClick(listing.id)}>
                  <AuctionImage>
                    ...
                    {(listing.status === 'active' || listing.status === 'upcoming') && (
                      <ShareButton onClick={(e) => handleShare(e, listing)} title="İlanı paylaş">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M7.217 10.907a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186zM12 5.25v.01M12 18.75v.01M16.883 10.907a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186zM6.117 6.117a2.25 2.25 0 100 4.186 2.25 2.25 0 000-4.186zM12 12.75a2.25 2.25 0 100-4.186 2.25 2.25 0 000 4.186z" />
                  ...
                  <AuctionContent>
                    ...
                    {(listing.ada_no || listing.parsel_no) && (
                      <AuctionMeta>
                        ...
                        {listing._display_type === 'auction' && listing.minIncrement > 0 && (
                          <MetaItem>
                            <IncrementIcon />
                            Min. Artış: <strong>{formatPrice(listing.minIncrement)}</strong>
                          </MetaItem>
                        )}
                      </AuctionMeta>
                    )}
                    ...
                    <AuctionDetails>
                      <PriceInfo>
                        <span>
                          {listing.status === 'active' ? 'Güncel Teklif' :
                           listing.status === 'ended' || listing.status === 'completed' ? 'Kapanış Fiyatı' :
                           'Başlangıç Fiyatı'}
                        </span>
                        <AuctionPrice>
                          {formatPrice(
                            listing.status === 'active' || listing.status === 'ended' || listing.status === 'completed'
                              ? getMinimumBidAmount(listing)
                              : (listing.starting_price || listing.startingPrice || listing.starting_bid || 0)
                          )}
                        </AuctionPrice>
                      </PriceInfo>
                      ...
                      {listing._display_type === 'offer' ? (
                        ...
                        <CountdownInfo>
                          ...
                          <CountdownTimer
                            endTime={new Date(listing.start_time || listing.startTime).toISOString()}
                            compact={true}
                          />
                        </CountdownInfo>
                      ) : (
