      ...
      for (const bid of userBidsData) {
        if (bid.auctions) {
          ...
          const auction = {
            ...bid.auctions,
            starting_price:
              bid.auctions.starting_price || bid.auctions.startingPrice,
            minIncrement:
              bid.auctions.min_increment || bid.auctions.minIncrement,
            ...
            images: Array.isArray(bid.auctions.images)
              ? bid.auctions.images
              : [],
          };
          ...
          if (
            ...
            bid.amount > uniqueAuctions[auction.id].highestBid
          ) {
            uniqueAuctions[auction.id] = {
              ...auction,
    ...
    <DashboardContainer>
      ...
      {error ? (
        ...
        <EmptyState>
          ...
          <EmptyStateMessage>
            Henüz hiçbir ihaleye teklif vermediniz. İhalelere göz atarak teklif
            verebilirsiniz.
          </EmptyStateMessage>
        </EmptyState>
      ) : (
        <div>
          <BidsList>
            {userBids.slice(0, 5).map((bid) => (
              <BidItem
                key={bid.id}
                onClick={() => handleAuctionClick(bid.auction.id)}
              >
                <BidItemContent>
                  <BidItemTitle
                    style={{
      ...
      {error ? (
        ...
        <EmptyState>
          ...
          <EmptyStateMessage>
            Şu anda aktif olan ve teklif verdiğiniz bir ihale bulunmamaktadır.
          </EmptyStateMessage>
        </EmptyState>
      ) : (
        <AuctionsGrid>
              ...
              <AuctionContent>
                ...
                {auction.end_time && (
                  <CountdownWrapper>
                    <CountdownTimer
                      endTime={
                        auction.end_time || auction.endTime || auction.end_date
                      }
                      compact={true}
      ...
      {error ? (
        ...
        <EmptyState>
          ...
          <EmptyStateTitle>
            Yaklaşan İhalede Teklifiniz Bulunmuyor
          </EmptyStateTitle>
          <EmptyStateMessage>
            Şu anda yaklaşan ve teklif verdiğiniz bir ihale bulunmamaktadır.
          </EmptyStateMessage>
        </EmptyState>
      ) : (
        <AuctionsGrid>
