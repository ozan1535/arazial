...
function AdminDashboard() {
  ...
  const handleDepositRefund = async (deposit) => {
    ...
    try {
      ...
      const { data: updateData, error: updateError } = await supabase.functions.invoke('update-deposit-status', {
        body: {
      ...
      if (updateError || !updateData.success) {
        console.error('Error updating deposit status:', updateError || updateData);
