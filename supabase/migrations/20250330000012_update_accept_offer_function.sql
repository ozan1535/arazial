-- migration:20250330000012

-- Drop the old function if it exists (both versions to be safe)
DROP FUNCTION IF EXISTS public.accept_offer(bigint);
DROP FUNCTION IF EXISTS public.accept_offer(uuid);

-- Create the updated function with UUID parameter
