-- migration:20250330000007

-- Update the complete_expired_auctions function to use 'ended' instead of 'completed'
...
BEGIN
    ...
    LOOP
        ...
        -- Update the auction status to ended and set the winner if there's a bid
...
-- Update the manual completion function to use 'ended' instead of 'completed'
...
'Automatically marks auctions as ended when they reach their end date/time, selecting the highest bidder as the winner.';

COMMENT ON FUNCTION public.complete_specific_auction(UUID) IS 
'Manually marks a specific auction as ended by ID, selecting the highest bidder as the winner.'; 